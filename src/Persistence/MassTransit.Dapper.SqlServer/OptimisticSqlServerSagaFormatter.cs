    ...
    where TModel : class
{
    ...
    public OptimisticSqlServerSagaFormatter(string? tableName = null, string? idColumnName = null, string? versionColumnName = null)
    {
        ...
        if (_versionColumnName is null)
            throw new InvalidOperationException($"Optimistic locking cannot be used with {type.Name} because the ROWVERSION column was not auto-detected.  Either specify the column name directly in the constructor, or ensure a public byte[] RowVersion property exists.");
    }
